...
export const ArchitectureEditor: React.FC<{
  ...
  onUpdate: (c: ScalingConcept[]) => void;
}> = ({ concepts = [], onUpdate }) => {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="border-b border-white/5 pb-4">
        ...
        <p className="text-slate-400 text-sm">
          Edit the scaling concepts (Scale Up, Scale Out, Scale Across).
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {concepts.map((item, idx) => (
          <div key={idx} className="bg-[#0d1117] p-6 rounded-2xl border border-white/5 space-y-4">
            ...
            <input
              value={claimText(item.desc)}
              onChange={(e) => onUpdate(arraySet(concepts, idx, 'desc', updateClaimText(item.desc, e.target.value)))}
              className="w-full bg-transparent text-sm font-bold text-blue-400 border-b border-white/10 outline-none"
              placeholder="Subtitle"
            />

            <textarea
              value={claimText(item.details)}
              onChange={(e) => onUpdate(arraySet(concepts, idx, 'details', updateClaimText(item.details, e.target.value)))}
