        ...
        {table.map((row, idx) => (
          <div key={idx} className="bg-[#0d1117] p-4 rounded-xl border border-white/5 grid grid-cols-12 gap-4 items-center">
            <div className="col-span-3">
              ...
              <input value={row.feature} onChange={e => { const next = [...table]; next[idx].feature = e.target.value; onUpdate(next); }} className="w-full bg-[#161b22] border border-white/10 rounded p-2 text-sm text-white font-bold" />
            </div>
            <div className="col-span-4">
              <label className="text-[10px] uppercase font-bold text-red-500/70 block mb-1">Legacy</label>
              <input value={row.legacy} onChange={e => { const next = [...table]; next[idx].legacy = e.target.value; onUpdate(next); }} className="w-full bg-[#161b22] border border-white/10 rounded p-2 text-sm text-slate-400" />
            </div>
            <div className="col-span-5">
              <label className="text-[10px] uppercase font-bold text-emerald-500/70 block mb-1">Modern (UEC)</label>
              <input value={row.pinnacle} onChange={e => { const next = [...table]; next[idx].pinnacle = e.target.value; onUpdate(next); }} className="w-full bg-[#161b22] border border-white/10 rounded p-2 text-sm text-emerald-100" />
            </div>
          </div>
        ))}
      </div>
